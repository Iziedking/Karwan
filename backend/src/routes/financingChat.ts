...
financingChatRoutes.post('/:kind/:positionId', async (c) => {
  ...
  const message = await addMessage({ id: `${params.data.positionId}-${Date.now()}-${randomBytes(4).toString('hex')}`, jobId: state.jobId, channel: 'financing', channelKey: params.data.positionId, financingKind: params.data.kind, financingId: params.data.positionId, sender: caller, kind: 'participant', body: text, ts: Date.now() });
