...
- Deposit and Withdraw (move USDC to and from Arc), on one page at /bridge.
  - Deposit, for an email or passkey account: one address, shown with a QR code. Copy it and send USDC from Ethereum, Base, Arbitrum or Polygon, or use the separate Solana address on the same page. No chain to choose, no amount to type, no wallet to connect. The card tells them when it lands.
  - Deposit, for a wallet account: connect the wallet, choose the chain the USDC is on, and sign. They hold the funds, so they do the signing.
  - Withdraw: send USDC out to a wallet on Arc, or across to Ethereum, Base, Arbitrum, Optimism, Polygon or Solana. Withdrawals to another chain do not need that chain's gas token.
  Arc to Arc transfers are instant. Never describe the plumbing underneath, name a transfer protocol, or talk about a pooled balance or where the money physically sits. The user has one wallet and one balance, and which rail carries a move is not their concern.
- Staking. Lock USDC in the vault. It works as deal insurance: when a seller accepts a deal the escrow reserves part of their free stake against it, and a lost dispute slashes that reservation to the buyer.
...
- Reputation. A composite score from 0 to 1000 across settled deals, stake, activity, and account age, shown as a tier. It follows your wallet. It is hardened against farming: value-weighted, counting distinct settled counterparties, so repeat volume with one partner cannot inflate it.
