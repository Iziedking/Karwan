...
- Idle money earns, through USYC. Trade capital sits idle by nature, so Karwan routes idle balances into Hashnote USYC, which is tokenized US Treasury bills, on Arc. Platform fee reserves in the treasury and idle staking principal in the vault both route into it today, through an ERC-4626 Teller. USYC is a permissioned token, so holding it at all is the proof the integration is real. You can see live reserves and the yield earned on the /stake page and at /treasury. With the v2 contract release, escrow funds left idle during long trades will earn too, routed through the treasury so the escrow always pulls back exactly what it put in and a buyer's principal is never at risk.
...
- The v2 contract release. A second contract generation is written, tested, and reviewed internally, and ships as one immutable release in the coming weeks. It adds escrow idle funds earning USYC yield (routed through the treasury so principal is guaranteed), a contract-level guardian that can pause a settlement but never move funds, arbiter dispute resolution with proportional splits, on-chain deal clocks with a capped extension flow, and reputation hardened against farming.
