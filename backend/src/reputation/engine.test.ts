...
test('a maxed wallet with no penalty reaches the top of the scale', () => {
  const result = compute(
    inputs({
      ...
      registeredAt: Date.now() - repConfig.tenureFullDays * 2 * DAY,
...
test('an empty wallet scores zero and is NEW', () => {
  const result = compute(inputs({ registeredAt: 0, lastActionAt: 0 }));
...
test('concentration caps the tier, and the advice names concentration not deals', () => {
  ...
  const result = compute(
    inputs({
      ...
      stakeUsdc: repConfig.stakeCapUsdc * 4,
      stakeDays: repConfig.stakeFullDays * 2,
      registeredAt: Date.now() - repConfig.tenureFullDays * 3 * DAY,
...
test('a ceiling never raises a tier', () => {
  ...
  const result = compute(inputs({ completedDeals: 20, totalStarted: 20 }));
...
test('a single flagged link lands most of the security penalty', () => {
  const one = compute(inputs({ securityOffenses: 1 }));
...
test('the decay half-life halves the score at the half-life, not before', () => {
  ...
  assert.ok(Math.abs(decayMultiplier(Date.now() - halflife * DAY) - 0.5) < 0.01);
  assert.ok(Math.abs(decayMultiplier(Date.now() - 2 * halflife * DAY) - 0.25) < 0.01);
  assert.equal(decayMultiplier(0), 1, 'a wallet with no last action is not decayed');
  assert.equal(decayMultiplier(Date.now() + DAY), 1, 'a future timestamp cannot boost a score');
});
...
test('every factor stays inside [0,1] for hostile inputs', () => {
  const hostile = inputs({
    ...
    registeredAt: Date.now() + 10 * DAY,
  });
  for (const [name, value] of Object.entries({
    stake: stakeScore(hostile),
    completion: completionScore(hostile),
    volume: volumeScore(hostile),
    tenure: tenureScore(hostile),
    activity: activityScore(hostile),
    referral: referralScore(hostile),
  })) {
...
test('factors are concave: the first unit is worth more than the tenth', () => {
  const first = volumeScore(inputs({ lifetimeVolumeUsdc: 50 }));
  const tenth = volumeScore(inputs({ lifetimeVolumeUsdc: 500 })) - volumeScore(inputs({ lifetimeVolumeUsdc: 450 }));
  assert.ok(first > tenth, 'volume should saturate');
  const firstDeal = completionScore(inputs({ completedDeals: 1, totalStarted: 1 }));
  const tenthDeal =
    completionScore(inputs({ completedDeals: 10, totalStarted: 10 })) -
    completionScore(inputs({ completedDeals: 9, totalStarted: 9 }));
...
test('staking pays something on day one', () => {
  ...
  const dayOne = stakeScore(inputs({ stakeUsdc: repConfig.stakeCapUsdc, stakeDays: 0 }));
...
test('a wallet that never staked gets no stake credit', () => {
  assert.equal(stakeScore(inputs({ stakeUsdc: 0, stakeDays: 90 })), 0);
});

test('completion rewards a clean record over a disputed one', () => {
  const clean = completionScore(inputs({ completedDeals: 8, totalStarted: 8 }));
  const messy = completionScore(inputs({ completedDeals: 8, totalStarted: 24 }));
