...
test('a report is counted only after successful delivery', async () => {
  ...
  const delivered = await store.commit({ reservationId: reserved.reservation.id, resultId: 'report-sha-1', now: 2_100 });
...
test('failed delivery releases its reservation without spending allowance', async () => {
  ...
  const released = await store.release({ reservationId: reserved.reservation.id, reason: 'provider unavailable', now: 2_100 });
...
test('delivery retry and a second agent cannot double-consume one report', async () => {
  ...
  const delivered = await store.commit({ reservationId: first.reservation.id, resultId: 'report-sha-1', now: 2_100 });
  const retry = await store.reserve({ agentAddress: AGENT_B, requestId: 'request-2', resourceId: 'deal:shared:seller', now: 2_200 });
  const committedAgain = await store.commit({ reservationId: retry.reservation.id, resultId: 'report-sha-1', now: 2_300 });
...
test('two verified agents share one human cap across distinct reports', async () => {
  ...
  for (const [index, agentAddress] of [AGENT_A, AGENT_B, AGENT_A].entries()) {
    ...
    await store.commit({ reservationId: reserved.reservation.id, resultId: `report-${index}`, now: 3_000 + index });
  }
...
test('a new UTC period starts a fresh allowance without deleting history', async () => {
  ...
  await store.commit({ reservationId: first.reservation.id, resultId: 'report-day-1', now: 86_399_500 });
