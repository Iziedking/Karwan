  ...
  return (
    <section className="flex min-h-[520px] min-w-0 flex-col overflow-hidden border border-[var(--lp-border-light)] bg-[var(--lp-card)] lg:h-[720px]" style={{ borderRadius: 16 }}>
      <header className="border-b border-[var(--lp-border-light)] px-4 py-4 sm:px-5">
        <p className="mono text-[9px] font-bold uppercase tracking-[0.15em] text-[var(--lp-text-muted)]">Seller ↔ financier</p>
        <h2 className="mt-1 text-lg font-semibold text-[var(--lp-dark)]">Private conversation</h2>
        <p className="mt-1 text-xs leading-5 text-[var(--lp-text-sub)]">Coordinate financing and repayment here. This chat is not shared with the buyer.</p>
      </header>

      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-4 sm:px-5">
        {messages.length === 0 ? <div className="mx-auto mt-12 max-w-xs text-center"><p className="text-sm font-medium text-[var(--lp-dark)]">Start the financing conversation</p><p className="mt-1 text-xs leading-5 text-[var(--lp-text-muted)]">Use this private space to confirm timing, delivery progress, and repayment questions.</p></div> : null}
      ...
      <footer className="border-t border-[var(--lp-border-light)] bg-white/40 p-3 sm:p-4">
        {writable ? <>
          {replyingTo ? <div className="mb-2 flex items-start justify-between gap-3 border-l-2 border-[var(--lp-dark)] bg-white/65 px-3 py-2"><div className="min-w-0"><p className="mono text-[8px] font-bold uppercase tracking-[0.12em] text-[var(--lp-text-muted)]">Replying to {roleFor(replyingTo.sender)}</p><p className="mt-0.5 truncate text-xs text-[var(--lp-text-sub)]">{excerpt(replyingTo.body)}</p></div><button type="button" onClick={() => setReplyingTo(null)} aria-label="Cancel reply" className="shrink-0 text-lg leading-none text-[var(--lp-text-muted)]">×</button></div> : null}
          <div className="flex items-end gap-2">
            <textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} rows={2} maxLength={2000} placeholder="Write a private message…" className="min-h-[48px] min-w-0 flex-1 resize-none border border-[var(--lp-border-light)] bg-white px-3 py-2.5 text-sm text-[var(--lp-dark)] outline-none focus:border-[var(--lp-dark)]" style={{ borderRadius: 10 }} />
        ...
        </> : <div className="rounded-lg border border-[var(--lp-border-light)] bg-white/55 px-3 py-3 text-xs leading-5 text-[var(--lp-text-sub)]"><strong className="text-[var(--lp-dark)]">Conversation closed.</strong> The full record remains available to the seller and financier.</div>}
      </footer>
    </section>
  );
}
