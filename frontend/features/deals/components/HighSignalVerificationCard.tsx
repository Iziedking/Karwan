    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 id="high-signal-title" className="mt-2 text-[18px] font-bold text-[var(--lp-dark)]">
            {verified ? 'World ID verification recorded' : 'Verify before this deal moves forward'}
          </h2>
          <p className="mt-2 max-w-[58ch] text-[13px] leading-relaxed text-[var(--lp-text-sub)]">
            {verified
              ...
              : 'This buyer selected an extra identity signal for a sensitive deal. Karwan keeps the proof result, not biometric data.'}
          </p>
        </div>
      ...
      {!verified ? (
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          >
            {busy ? 'Preparing…' : unavailable ? 'Retry World ID check' : 'Prepare World ID check'}
          </button>
          <span className="text-[11px] text-[var(--lp-text-muted)]">You complete the credential step in a supported World client.</span>
        </div>
      ) : null}
      {request ? (
        <div className="mt-4 space-y-2 rounded-[10px] border border-[var(--lp-border-light)] p-3">
          ...
          <p className="text-[12px] leading-relaxed text-[var(--lp-text-sub)]">
            Open the World credential client with action <strong>{request.action}</strong>. The challenge expires at {new Date(request.expires_at * 1000).toLocaleTimeString()}.
          </p>
        </div>
      ) : null}
      {request && rpContext ? (
