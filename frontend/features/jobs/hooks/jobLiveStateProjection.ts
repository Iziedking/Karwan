...
export interface JobLiveStateProjection {
  events: ChainEvent[];
  active: StepKey;
  completed: StepKey[];
  declined: boolean;
  ...
  /**
   * Recoverable negotiation pause. A bounded attempt can exhaust without
   * closing the underlying request; a later material trigger may resume it.
   */
  recoverable:
    | 'temporary_impasse'
...
/**
 * Derive the buyer flow state from a snapshot and ordered live events.
 *
 * The live-event hook supplies newest-first rows; the reducer reverses a copy
 * so transitions are applied oldest-first. This is intentionally a pure
 * projection: it does not fetch, persist, or mutate either input. Keeping the
 * reducer outside the hook makes terminal and recoverable-state rules directly
 * characterizable in unit tests.
 */
export function deriveJobLiveState(
  initial: BuyerJob,
  ...
  for (const e of [...events].reverse()) {
    ...
    if (e.type === 'negotiation.exhausted') {
      // Exhausting one bounded attempt is not a permanent rejection. The
      // request remains open for a material trigger or explicit review.
      recoverable = 'temporary_impasse';
      continue;
    }
    if (
      e.type === 'qualification.blocked' ||
      e.type === 'stake.required' ||
      e.type === 'stake.funding.required' ||
      e.type === 'approval.requested' ||
      e.type === 'approval.expired'
    ) {
      recoverable = 'needs_approval';
      continue;
    }
    if (
      e.type === 'evidence.pending' ||
      e.type === 'evidence.unknown' ||
      e.type === 'financial.provider.unknown' ||
      e.type === 'financial.reconciling'
    ) {
      recoverable = 'status_updating';
      continue;
    }
    ...
    if (!s) continue;
    if (s === 'bidding' || s === 'counter' || s === 'accepted') recoverable = null;
    seen.add(s);
  ...
  if (initial.escrowFunded || ended !== null || outOfReachActive) recoverable = null;
  ...
  return { events: [...events], active: highest, completed, declined, ended, recoverable, outOfReach };
}
